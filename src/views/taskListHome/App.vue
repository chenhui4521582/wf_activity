  ...
  <div id="app">
    ...
    <div class="task-wrap">
      <div v-for="(item,index) in allTaskList" :style="{'margin-bottom':'0.2rem'}">
        <crushTask v-if="showCrushMasterTask(item)" :item="item" :index="index" :platSource="platSource" @showCurDetails="showCurDetails" @receive="receive" @finish="finish" />
        <kingTask v-if="showKingTask(item)" :item="item" :index="index" :platSource="platSource" @showCurDetails="showCurDetails" @receive="receive" @finish="finish" />
        <king-no-lock :item="item" :platSource="platSource" v-if="showKingTaskNoLock(item)" />
      </div>
      <div v-for="(item,index) in finishList" :style="{'margin-bottom':'0.2rem'}">
        <crushTask v-if="showCrushMasterTask(item)" :item="item" :index="index" :platSource="platSource" :type="'finishList'" @showCurDetails="showCurDetails" @receive="receive" @finish="finish" />
        <kingTask v-if="showKingTask(item)" :item="item" :index="index" :platSource="platSource" :type="'finishList'" @showCurDetails="showCurDetails" @receive="receive" @finish="finish" />
      </div>
...
export default {
  ...
  computed: {
    ...
    platSource () {
      return this.getUrlParam('from')
    }
  },
  methods: {
    async getMasterTaskNameList () {
      let { data } = await this.axios.post('//platform-api.beeplay123.com/task/api/usertask/achievementTaskList')
      if (data.code == 200) {
        await GLOBALS.marchSetsPoint('P_H5PT0122', { entrance: this.platSource })
    ...
    async receive (item, type, val, curParentTask) {
      ...
      let { data } = await this.axios.post('//platform-api.beeplay123.com/task/api/usertask/finish', {
        taskId,
        taskLogId
      })
    ...
    async getMasterList (val, type, item, otherStatus) {
      let { data } = await this.axios.post('//platform-api.beeplay123.com/task/api/usertask/achievementTaskInHall', { value: val })
