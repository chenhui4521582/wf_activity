<template>
  ...
  >
    ...
    <!-- 攻略,金币记录,往期榜单(无光背景) -->
    <!-- 太棒了（有光背景） -->
    <template v-if="value===2||value===3||value===4||value===5||value===6||value===7||value===8||value===9">
      <div v-if="value===2||value===3||value===4||value===5||value===6||value===7||value===8||value===9" class="nolight-bg" :class="value===5||value===6||value===7||value===8||value===9?'light-bg':''">
        ...
        <template v-if="value===4">
          <div class="topedlist-container">
            <div class="topedlist-tab">
              <!-- 昨日榜单按钮 -->
              <div class="yesterday-btn" @click="currenttoplist='yesterday'" :style="{backgroundColor:currenttoplist==='toped'?'transparent':''}">
                <img src="../images/compop/yesterday-btn.png" alt />
              </div>
              <!-- 往期榜单按钮 -->
              <div class="toped-btn" @click="currenttoplist='toped'" :style="{backgroundColor:currenttoplist==='toped'?'#FF5B57':''}">
            ...
            <template>
              <div class="toped-tab-container">
                <ul>
                  <!--  昨日榜单-->
                  <template v-if="currenttoplist==='yesterday'">
                    <li>
                      <div>排名</div>
                      <div>昵称</div>
                      <div>金币数</div>
                    ...
                    <div class="toped-list-inner topthreecss">
                     ...
                     <li  v-for="(item,index) in otherData" :key="'inf'+index">
                      ...
                      <li v-if="!isOpen">
                        <img @click.stop="closeOpenProfit" class="clickshow" src="../images/compop/clickshow.png" alt="">
                      </li>
                  ...
                  <template v-if="currenttoplist==='toped'">
                    <template v-if="rankingList.length>0">  <li>
                      <div>日期</div>
                      <div>排名</div>
                      <div>当日金币数</div>
                      <div>奖励</div>
        ...
        <template v-if="value===5||value===6||value===7||value===8||value===9">
          ...
          <img v-if="value===7&&activityInfo.awardsType==='jdk'||value===8&&activityInfo.awardsType==='jdk' " src="../images/compop/jdk.png" alt="">
              <img v-if="value===7&&activityInfo.awardsType==='hfq'||value===8&&activityInfo.awardsType==='hfq'" src="../images/compop/hfq.png" alt="">
               <img v-if="value===9" src="../images/compop/pass.png" alt="">
              <!-- 弹窗下面文字提示 -->
            <template v-if="value===5||value===6">
              <p  v-if="value===5" class="purpletexts onerowtips">恭喜您获得<span class="orangetexts">{{activityInfo.awardsNum}}个金币</span></p>
              <p v-if="value===6" class="purpletexts onerowtips">昨日获得<span class="orangetexts">{{activityInfo.awardsNum}}金币</span>，暂未上榜哦~</p>
              <div class="iknow" @click="close"></div>
            </template>
            <!-- 恭喜您上富翁榜弹窗下方文字提示 -->
            <template v-if="value===7">
              <p class="purpletexts  congra-ontop">昨日富翁榜<span class="orangetexts">第{{activityInfo.awardsNum}}名</span>,<br>获得<span class="orangetexts" >{{activityInfo.propAwardsName}}</span></p>
          ...
          <template v-if="value===8">
            <p class="purpletexts endtop" >终极土豪榜<span class="orangetexts">第{{activityInfo.awardsNum}}名</span>,<br>获得<span class="orangetexts">{{activityInfo.propAwardsName}}</span></p>
          </template>
<!-- 哎呀（富翁榜被反超） -->
...
export default {
 ...
 mounted() {
  ...
  computed:{
    //     topThreeStartList () {
    //   let arr = this.yesterdayList.filter((currentValue, index, arr) => {
    //     return index >=0 && index < this.defaultIndex
    //   })
    //   return arr
    // },
    //     otherList () {
    //   let arr = this.yesterdayList.filter((currentValue, index, arr) => {
    //     return index >= this.defaultIndex && index < this.yesterdaylist.length - 2
    //   })
    //   return arr
    // },
    //     lastStartList () {
  ...
  methods: {
     ...
     console.log(this.yesterdayList,this.rankingList)
          if (this.yesterdayList.length > 3) {
            this.lastTwoData = this.yesterdayList.slice(3)
          }
          if (this.yesterdayList.length > 6) {
            this.isOpen = false
            this.lastTwoData = this.yesterdayList.slice(this.yesterdayList.length - 2)
            this.otherData = this.yesterdayList.slice(3, this.yesterdayList.length - 2)
          }
