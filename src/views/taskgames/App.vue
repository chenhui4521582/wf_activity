  ...
  <div class="task-games">
    <div class="header">
      <ul>
        ...
        <li class="hf-fragment" v-if="telFragment" @click="jumpMine">
          {{telFragment&&telFragment[0].price}}
      ...
      <entrance-header @gotokf="kfclick" @getUserInfo="getTransInfo" :userInfo="userInfo">
      </entrance-header>
    </div>
    <!-- sdk 推广 -->
    ...
    <!-- sdk 新版运营位 -->
    <sdk-tab-box :currentGameType='currentGameType' :id="userInfo&&userInfo.userId"
      @kickegg="kickegg">
      <div>
        <div class="t-content" v-if="!isTfStatus">
          ...
          <new-user-task :newTaskItems="newTaskItems" :motherTask="motherTask"
            :newUserTaskobj="newUserTaskobj" :channel="channel" @receive="receive"
            ...
            <div class="new-task-header">
              <div class="new-task-inner">
                <h4 class="h-title h-new-title icon-tips">
                  ...
                  <div class="text"><img
                      src="./images/cloak.png">{{newTaskItems.countDown | formatTime}}</div>
                </h4>
                <div class="newTask">
                  ...
                  <ul>
                    <li class="percent-lq">
                      <div class="percent-box">
                        <em
                          :style="{width: motherTask.hasFinishedNum/motherTask.allTaskNum * 100 + '%' }"></em>
                      </div>
                    </li>
                  </ul>
                  <div class="explain">
                    <div class="text">{{motherTask.hasFinishedNum}}/{{motherTask.allTaskNum}}</div>
                    <div v-show="newTaskItems.receiverCounter" class="receive">
                      已有{{newTaskItems.receiverCounter}}人领取</div>
                  </div>
                </div>
              </div>
            </div>
            <div v-if="motherTask.hasFinishedNum != motherTask.allTaskNum">
              <ul class="t-items">
                <li class="not-daily">
                  <div :class="{'actived': newUserTaskobj.taskStatus == 2}">
                    ...
                    <div class="item-text">
                      ...
                      <div class="percent-container">
                        <div class="percent-box">
                          <div class="text">{{newUserTaskobj.finishNum}}/{{newUserTaskobj.taskOps}}
                          </div>
                          <em
                            :style="{width:newUserTaskobj.finishNum/newUserTaskobj.taskOps * 100 + '%'}"></em>
                        </div>
                  ...
                  <p class="btn-box">
                    <a href="javascript:" class="btn btn-receive"
                      v-if="newUserTaskobj.taskStatus == 0"
                      @click="receive(newUserTaskobj, 'newtask')">领取</a>
                    <a href="javascript:" class="btn btn-play" v-if="newUserTaskobj.taskStatus == 1"
          ...
          <fixed-entrance @checkTaskStatus="goFinish" @close="closeFixedEntrance"
          ...
          <template v-if="isShowOther">
            <div class="wrap">
              ...
              <div class="wrapper" v-if="MasterCostDown">
                <!-- 大师任务 -->
                <crush-master-task v-if="showCrushMasterTask " :crushTaskList="crushTaskList"
                  :showReceiveMedal="showReceiveMedal" :showMedalAnimate="showMedalAnimate"
                  :currentMedalIndex="currentMedalIndex" :currentGameType="currentGameType"
                  @checkTaskStatus="checkTaskStatus" @hideMedalAnimate="showMedalAnimate = false"
                  @receive="receive" @refreshTask="refreshTask" />
                <!-- 王者任务 -->
                <king-task v-if="showKingTask" :crushTaskList="crushTaskList"
                  :showReceiveMedal="showReceiveMedal" :showMedalAnimate="showMedalAnimate"
                  :currentMedalIndex="currentMedalIndex" :currentGameType="currentGameType"
                  @checkTaskStatus="checkTaskStatus" @hideMedalAnimate="showMedalAnimate = false"
                  @receive="receive" @refreshTask="refreshTask" />
          ...
          <div
            v-if="currentGamesItems&&currentGamesItems.length && newTaskItems&&(isShowOther||currentGamesItems[0].flag)">
            ...
            <div class="complete-task" v-if="completeData.show">
              <div style="display: flex;align-items: center;">
                ...
                <div class="item-text">
                  ...
                  <div class="percent-container">
                    <div class="percent-box">
                      <div class="text">{{completeData.userTaskFinishNum}}/{{completeData.taskNum}}
                      </div>
                      <em
                        :style="{width:completeData.userTaskFinishNum/completeData.taskNum * 100 + '%'}"></em>
                    </div>
                  </div>
                </div>
              </div>
              <p class="btn-box">
                <a href="javascript:" class="btn btn-play1" v-if="completeData.userTaskFinishAll"
            ...
            <ul class="t-items">
              <li v-for="(item, index) in currentGamesItems" :key="index">
                ...
                <p class="btn-box">
                  <a href="javascript:" class="btn btn-receive" v-if="item.taskStatus == 0"
                    @click="checkTaskStatus(item,'dayTask',index)">领取</a>
                  <a href="javascript:" class="btn btn-play" v-if="item.taskStatus == 1"
              ...
              <ul class="t-items">
                <li v-for="(item, index) in otherGamesItems" :key="index">
                  ...
                  <p class="btn-box">
                    ...
                    <a href="javascript:" class="btn btn-gray" v-if="item.taskStatus == 2"
          ...
          <img src="./img/weekbetting.png" alt="" class="weekbetting"
            style="display: block;width: 100%;margin: .2rem 0" @click="gotoweekbetting"
            v-if="showWeekBetting">
          <poplog v-if="isPopLog" :crushTaskList="crushTaskList" :awardItem="awardItem"
            :motherTask="motherTask" :isNewTask="isNewTask" :masterTask="masterTask"
            :newUserTaskFinish="newUserTaskFinish" @close="closePopLog">
          </poplog>
          <!-- 新版奖励弹窗   -->
          ...
          <!-- 踏青寻宝   活动特有  活动下线 删除-->
          <box-dialog v-if="showBoxDialog" :awardItem="awardItem"
    ...
    <common-pop title="温馨提示" @close="showzspop=false" btn-name="确定" v-if="showzspop"
      @besure="gotokf">
