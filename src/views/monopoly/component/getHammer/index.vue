<template>
  <div class="get-hammer">
    <template v-if="taskProgressInfoData">
      <template v-for="(itemtitle,indextitle) in $moduleConfig.monopoly.dropDown.inner.info.titles">
        <h4 class="s-title"><span><em></em>{{itemtitle}} </span><span class="next"
            v-if="indextitle==0">下一梯度解锁需支持金叶：<i>{{taskProgressInfoData.gameProgress.diffNextProgress| conversion}}</i></span>
        </h4>
        <template v-if="indextitle==0">
          <span class="info">{{$moduleConfig.monopoly.dropDown.inner.info.sideTitle}}</span>
          <div class="g-package"
            :style="{background:$moduleConfig.monopoly.dropDown.inner.tabs.btnDefaultStyle.background}">
            ...
            <div class="g-package-container g1">
              <ul>
                <li v-for="(item,index) in mallBizConfigs" @click="gotopay(item)"
                  :style="{background:$moduleConfig.monopoly.dropDown.inner.packageBlockBg}">
                  <img :src="$moduleConfig.monopoly.dropDown.inner.packageImgs[index]" alt="">
                  <div class="item-text"
                    :style="{color:$moduleConfig.monopoly.dropDown.inner.packageBlockTextColor}">
                    {{item.content.split('+')[0]}}<br />
                    <span class="content"
                      :style="{color:$moduleConfig.monopoly.dropDown.inner.packageBlockBtnBg,display: 'flex',alignItems: 'center',justifyContent: 'center'}">
                  ...
                  <a href="javascript:" class="btn-price"
                    :style="{background:$moduleConfig.monopoly.dropDown.inner.packageBlockBtnBg}">￥{{item.price}}</a>
                </li>
              </ul>
            </div>
            <div class="g-package-info">
              <ul class="li0">
                ...
                <!--<li><div class="line" :style="{background:$moduleConfig.monopoly.dropDown.inner.tabs.btnDefaultStyle.color}"></div>获得幸运币：{{taskProgressInfoData.buyProgress.returnNum}}个</li>-->
              </ul>
            </div>
          </div>
        </template>
