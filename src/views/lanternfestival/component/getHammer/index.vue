<template>
  <div class="get-hammer">
    <template v-if="taskProgressInfoData">
      <h4 class="s-title"><em></em>每日任务送瓜分券（每日重置）</h4>
      <div class="task_container" v-for="(item,index) in taskProgressInfoData.opsActivityTaskProgressRsps">
        <div class="item">
          <p v-if="index==0">每天完成{{item.totalNum}}个每日任务给1张券</p>
          <p v-else-if="index==1">今日累计充值{{item.totalNum}}元给1张券</p>
        </div>
        <div class="item">
      ...
      <h4 class="s-title"><em></em>玩游戏得瓜分券（每日重置）</h4>
      <span class="info">在游戏中(除套圈，枪火英雄，麻将、斗地主、生肖大作战、飞机大战及跑得快)消耗金叶可以获得对应数量的瓜分券</span>
      <div class="g-package">
        ...
        <div class="g-package-info">
          <ul>
            ...
            <li>今日获得瓜分券：{{taskProgressInfoData.gameProgress.receiveNum }}张</li>
          </ul>
        </div>
      </div>
      <h4 class="s-title"><em></em>购买礼包得瓜分券</h4>
      <div class="g-package">
        ...
        <div class="g-package-info">
          <ul>
            <li>今日已购买礼包：{{taskProgressInfoData.packageRsp.packageBetting}}次</li>
            <li>今日获得瓜分券：{{taskProgressInfoData.packageRsp.packageReceiveNum}}张</li>
          </ul>
        </div>
      </div>
    </template>
  ...
  export default {
    ...
    methods: {
      gotopay(item) {
        ...
        GLOBALS.marchSetsPoint('A_H5PT0240002799', {recharge_rmb: item.price, product_id: item.bizId})   // H5平台-砸金蛋-获取瓜分券大浮层-点击任意礼包
      ...
      async gain(item, index) {
        GLOBALS.marchSetsPoint('A_H5PT0240002796', {
          task_id: index + 1,
          task_name: index == 0 ? `每天完成${item.totalNum}个每日任务给1张券` : `今日累计充值${item.totalNum}元给1张券`
        })   // H5平台-元宵活动-获取瓜分券-每日任务奖励领取点击
        const {code} = await taskReceive({sort: index + 1})
        if (code === 200) {
          this.awardData = {
            awards: 1
          }
          this.showPop({
            type: 6, awardData: this.awardData
          })
          this.refresh(false)
        }
      },
