<template>
  <div class="animation" v-if="showAnimation">
    <div class="wrapper">
      <div class="groups animation-ease" v-for="(item, index) in animationList" :key="index">
        <div class="status">
          ...
          <div class="status3" v-if="item.state == 3">
            ...
            <div class="btn" v-if="userInfo.nextStage == item.stage"><img
                src="../images/lock-btn.png" alt=""></div>
        ...
        <div class="groups-wrapper" ref="groups" @webkitTransitionEnd="endGame()">
          <ul class="group-item" ref="groupItem" v-for="(item, index) in round" :key="index">
            <li :class="`prize-item prize-item${item}`" ref="item" v-for="(item, index) in prizes"
              :key="index">
    ...
    <popup v-model="showPopup" :status="status" :AwardsList="AwardsList"
      :AwardsNumber="AwardsNumber" :userInfo="userInfo"></popup>
  </div>
</template>
<script>
...
.animation {
  position: relative;
  margin: 0.48rem 0.65rem 0 0.5rem;
  height: 1.97rem;
  .wrapper {
    overflow: hidden;
    height: 1.97rem;
    .groups {
      ...
      width: 0.82rem;
      margin-left: 0.05rem;
      background: url(../images/number-bg-active.png) no-repeat center top /
        100% 100%;
      .status {
        position: absolute;
        left: 0;
        ...
        .status2 {
          ...
          width: 100%;
          height: 1.97rem;
          background: url(../images/number-bg.png) no-repeat center top / 100%
            100%;
          .lock {
      ...
      &:first-child {
        ...
        overflow: hidden;
        background: url(../images/number-right.png) no-repeat center top / 100%
          100%;
      }
      &:last-child {
        border-radius: 16px 0 0 16px;
        background: url(../images/number-left-active.png) no-repeat center top /
          100% 100%;
        overflow: hidden;
        margin-left: 0;
        .status3 {
          background: url(../images/number-left.png) no-repeat center top / 100%
            100%;
        }
      }
      .groups-wrapper {
