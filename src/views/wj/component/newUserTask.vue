    ...
    <div class="new-task-header">
      <div class="new-task-inner">
        <h4 class="h-title h-new-title icon-tips">
          ...
          <div class="text"><img src="../img/clock.png">{{newTaskItems.countDown | formatTime}}</div>
        </h4>
        <div class="newTask">
      ...
      <ul class="t-items">
        <li class="not-daily" v-for="(item,index) in newUserTaskList" :key="index">
          <div class="item-content">
            ...
            <p class="btn-box">
              <a href="javascript:" class="btn btn-receive" v-if="item.taskStatus == 0" @click="receive(item,index)">领取</a>
              <a href="javascript:" class="btn btn-play" v-if="item.taskStatus == 1" @click="goFinish(item,index)">去完成</a>
...
export default {
  ...
  methods: {
    receive (item, index) {
      this.$emit('receive', item, 'new_user_task', index)
    },
    goFinish (item, index) {
      this.$emit('goFinish', item, 'new_user_task', index)
    },
    getList () {
      this.$emit('getList')
    }
