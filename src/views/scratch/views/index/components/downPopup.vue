    ...
    <transition name="fade">
      <div class="drop-down" v-if="curIndex">
        <div class="drop-down-content" :style="$moduleConfig.superLotto.dropDown.inner.bg">
          <div class="d-tab" :style="$moduleConfig.superLotto.dropDown.inner.tabBg">
            <ul style="height: 100%">
              <li 
                v-for="(item,index) in $moduleConfig.superLotto.dropDown.inner.tabs.btnText"
                :key="index"
                :style="$moduleConfig.superLotto.dropDown.inner.tabs[curIndex - 1 === index?'activeStyle':'btnDefaultStyle']"
                @click="innerHandleTab(index)"
              >
                {{item}}
              </li>
            </ul>
