<template>
  <div>
          ...
          <div class="d-tab" :style="$moduleConfig.viruskill.dropDown.inner.tabBg">
            <ul style="height: 100%">
              <li @click="innerHandleTab(index)"
                  v-for="(item,index) in $moduleConfig.viruskill.dropDown.inner.tabs.btnText" :style="$moduleConfig.viruskill.dropDown.inner.tabs[curIndex == index?'activeStyle':'btnDefaultStyle']">{{item}}
              </li>
            </ul>
          </div>
          <!-- 获取幸运币组件 -->
    ...
    <div class="bottom-wrap" v-if="!isDropDown" :style="$moduleConfig.viruskill.dropDown.out.bg">
      <div class="tab" @click="outHandleTab(index)" v-for="(item,index) in $moduleConfig.viruskill.dropDown.out.tabs">
        <div class="hammer_icon" :style="item.icon"></div>
        <div class="right">
          <div class="main-text" v-if="item.mainText">已得<span>{{toDayUserCouponNum|filterPrice}}</span>个币</div>
          <div class="btn" :class="{hasMainTxt:item.mainText}" :style="item.btnStyle">{{item.btnText}}</div>
        </div>
      </div>
