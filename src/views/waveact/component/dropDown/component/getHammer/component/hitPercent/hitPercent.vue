    ...
    methods: {
      ...
      getGameProgress() {
        Services.gameProgress().then((res) => {
          if (res.data.code == 200) {
            this.hbItems = res.data.data.gameProgress.progressList
            this.gameBetting = res.data.data.gameProgress.gameBetting
            let minUnfinished = this.hbItems.filter((item) => {
              return item.status == 0
            }).sort((a, b) => {
              return a.amount - b.amount
            ...
            this.$emit('gameinfo', {
              ...
              receiveNum: res.data.data.gameProgress.receiveNum,
              nextLeafs:minUnfinished&&minUnfinished.amount||0
            })
            // 下面是测试数据
