<template>
  <!--窗口类型 0 领取昨日幸运币奖励 1 活动规则 2 瓜分记录 3 瓜分攻略 4 大家都在玩 5 开榜 6 任务流水领取弹窗-->
          ...
          <div :class="getClassName('main')">
            <div :class="getClassName('container_compop')">
              <template v-if="popType==0||popType==6">
                ...
                <h4 v-else>恭喜您获得{{awardData.awards}}个幸运币</h4>
                <h2 v-if="popType==0">您昨日共获得{{awardData.couponNum}}个幸运币</h2>
                <h2 v-else>幸运币越多，奖励越好哦！</h2>
                <div class="btn" v-if="popType==0" @click="$emit('getCouponAward')">立即领取</div>
                <div class="btn" v-else @click="getmore">获取更多幸运币</div>
                <div class="tips" v-if="popType==0">幸运币越多，奖励越好哦！</div>
              ...
              <template v-else-if="popType==1">
                <scroll :data="ruleTime">
                  <div>
                    ...
                    <p>2. 玩家在游戏中消耗一定金叶子，可以领取对应额度的幸运币，<i>金叶消耗进度每天清零</i></p>
                    <p>3. 瓜分奖励计算：5千万金叶子奖池÷当日活动累计发出的幸运币数量x玩家手上的幸运币，即为每人可以瓜得的奖励 </p>
                    <p>4. <i>每日奖励需要玩家次日在活动页领取，超过24小时不领，则视为放弃奖励</i></p>
                    <p>5. 幸运币每日0点清零重新计算</p>
                    <p>6. 活动期间内，玩家每天获得的幸运币数量会计入排行榜，活动结束后根据7日累计获得的券数量进行排名并发放排行奖励</p>
                    <p>7. 活动结束后，排行榜会展示1天时间，排行榜奖励将直接到账</p>
                    <p>活动结束后：</p>
                    <p>1. 未领取的幸运币作废，请及时领取</p>
                ...
                <scroll>
                  <div>
                    <div class="gonglueitem">
                      ...
                      <div class="right">
                        <h4>获得幸运币:</h4>
                        <h2>在指定游戏中消耗金叶子，充值礼包，完成每日任务可以获得幸运币</h2>
                      </div>
                    </div>
                    <div class="gonglueitem">
                      ...
                      <div class="right">
                        <h4>每天瓜分千万奖池:</h4>
                        <h2>次日0点后开启奖池，根据玩家手中的幸运币数量进行奖池瓜分</h2>
                        <h4>瓜分奖励计算:</h4>
                        <h2>5千万金叶子奖池÷当日活动累计发出的幸运币数量x玩家手上的幸运币，即为每人可以瓜得的奖励（详见规则）</h2>
                      </div>
                    </div>
                    <div class="gonglueitem">
                      ...
                      <div class="right">
                        <h4>上榜赢大奖：</h4>
                        <h2>玩家每天获得的幸运币数量会计入排行榜，最高可得价值35000元大奖</h2>
                      </div>
                    </div>
                  </div>
                </scroll>
              ...
              <template v-else-if="popType==5">
                ...
                <div class="tips" v-if="!isHasGift">啊哦！您昨日未获得幸运币</div>
              </template>
            </div>
          </div>
        </div>
