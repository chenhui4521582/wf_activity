...
var ScoreToast = {} // 定义插件对象
ScoreToast.install = function (Vue, options) { // vue的install方法，用于定义vue插件
 // 如果toast还在，则不再执行
 if(document.getElementsByClassName('scoreBox').length){
   return
 }
 let toastScore = Vue.extend(Alert) // 创建vue构造器
 ...
 // })
 let $vm = new toastScore() // 实例化vue实例
 // 此处使用$mount来手动开启编译。用$el来访问元素，并插入到body中
 let tpl = $vm.$mount().$el
 document.body.appendChild(tpl)

 Vue.prototype.$score = { // 在Vue的原型上添加实例方法，以全局调用
