<template>
  <div class="get-hammer">
    <h4 class="s-title"><em></em>玩游戏得游戏币</h4>
    <div class="g-package">
      <div class="g-package-container g2">
        <hit-percent :detail-data="myInfo" @gameinfo="getGameInfo" @refresh="refresh"></hit-percent>
      </div>
    ...
    <h4 class="s-title"><em></em>每日任务送抽奖机会</h4>
    <div class="task_container" v-if="taskProgressInfoData">
      <div class="item">
        <p>每天完成{{taskProgressInfoData.totalNum}}个每日任务可以获得一个游戏币</p>
        <p><i>（每天仅可完成一次哟）</i></p>
      </div>
      <div class="item">
        <div class="btn btn_complete" v-if="taskProgressInfoData.state==0" @click="gotocomplete">去完成</div>
        ...
        <div class="btn_progress">{{taskProgressInfoData.finishNum}}/{{taskProgressInfoData.totalNum}}</div>
      </div>
    </div>
  ...
  export default {
    ...
    methods: {
      gotopay(item) {
        ...
        GLOBALS.marchSetsPoint('A_H5PT0229002673', { recharge_rmb: item.price, product_id: item.bizId,product_name:item.content })   // HH5平台-双旦活动页-获得抽奖机会页-超值礼包-礼包点击
      ...
      async getShowLeaguePacksList() {
        const {code, data} = (await Services.showLeaguePacksList()).data
        if (code === 200) {
