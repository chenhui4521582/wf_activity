<template>
  <div id="app" :class="{aoke:curChannel==100006&&isHasIframe}">
    ...
    <template v-if="detailData&&detailData.showFlag">
      <div class="section0">
        <span v-if="detailData&&!detailData.activityEndFlag">截止时间：{{detailData.endDateTime}}</span>
        <span v-if="detailData&&detailData.activityEndFlag">活动已结束，未领奖励限今日领完</span>
      </div>
      <!--签到-->
      <div class="section1" id="section1"
        v-if="signData&&[2500,5000,10000,25000].includes(signData.count) && detailData && !detailData.activityEndFlag">
        <img class="title_bg" :src="require(`./images/package/signtab_${signData.count}.png`)">
        <div class="container">
          <ul class="signlist">
            <li v-for="(item,index) in signData.signInfoList">
              <img :src="require(`./images/package/${signData.count/5}.png`)" alt=""
                v-if="!item.status">
      ...
      <div class="section3" id="section3"
        v-if="bonusData&&[35,70,175,340].includes(bonusData.totalAwardsNum)"
        :class="{hasbtn:bonusData&&bonusData.pointsExchangeTaskVOS&&bonusData.pointsExchangeTaskVOS[0]&&bonusData.pointsExchangeTaskVOS[0].status!=1}">
        <img class="title_bg"
          :src="require(`./images/package/dailytasktab_${bonusData.totalAwardsNum}.png`)">
        <div class="container">
          <div class="info">
          ...
          <div class="bonus">
            <ul>
              <li v-for="(item,index) in bonusData.pointsExchangeTaskVOS">
                ...
                <div class="award_status" :class="{gained:item.status==2}" @click="exchange(item,3)"
                  v-if="item.status!=1">{{item.status==0?'领取':''}}</div>
              </li>
            </ul>
            <div class="progress">
              <div class="progress-bar">
                <div class="progress-bar-len"
                  :style="{width:getpercent(bonusData.pointsExchangeTaskVOS,bonusData.points)}">
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="tabs">
          <ul>
            <li v-for="(item,index) in gamesArr"
              :class="{actived:currentGameTask==index,redDot:isRedDot(item)}"
              ...
              <div class="info">
                ...
                <div class="progress">
                  <div class="progressbar">
                    <div class="progress-bar-len"
                      :style="{width:item.finishNum/item.taskOps * 100 + '%'}">
      ...
      <div class="section2" id="section2"
        v-if="detailData&&!detailData.activityEndFlag&&packageData&&!packageData.haveBuy&&packageData.list.length"
        :class="{package4:packageData&&packageData.list.length>2}">
        <div class="container">
          <img :class="{item1:!index}" :src="item.icon|filter" alt=""
            v-for="(item, index) in packageData.list" :key="index" @click="goPay(index,item)">
        </div>
      </div>
      <!--任务-->
      <!--充值加赠-->
      <div class="section5" id="section5" v-if="packageData&&packageData.haveBuy">
        <div class="container">
          ...
          <div class="text"
            style="text-align:center;width:100%;font-size:0.3rem;font-weight:bold;color:rgba(247,172,91,1);line-height:0.36rem;">
            {{packageData.dateRange}}连续登录<br /> 分天到账加赠部分</div>
        </div>
      </div>
      <!--活动结束-->
      <div class="section6" id="section6_1"
        v-if="packageData&&!packageData.haveBuy&&detailData&&detailData.activityEndFlag">
      ...
      <div class="section6" id="section6_2" v-if="detailData&&detailData.activityEndFlag">
        <img class="title_bg" v-if="signData&&[2500,5000,10000,25000].includes(signData.count)"
          :src="require(`./images/package/signtab_${signData.count}.png`)">
        <div class="container">
          <img src="./images/package/end.png" alt="">
        </div>
      </div>
    ...
    <!--奖品弹窗-->
    <award-pop v-if="showAwardPop" :awardsname="awardsname" :type="awardType" @close="closeAwardPop"
      @still-buy="stillBuy"></award-pop>
  </div>
</template>
<script>
...
.section0 {
  ...
  span {
    ...
    &::after {
      ...
      width: 0.16rem;
      height: 0.16rem;
      background: url('./images/package/star.png') no-repeat center center /
        100% 100%;
      margin-top: -0.08rem;
    }
    &::before {
