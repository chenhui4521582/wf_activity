        ...
        <div :class="getClassName('wrap')" @click="close(from)">
          <div class="title">
            <template v-if="[1,2].includes(from)">
              <img class="fabang-title" src="../images/pop/fabang-title.png" alt />
            </template>
            <template v-else-if="from==3">
              <img class="sorry-title" src="../images/pop/sorry-title.png" alt />
            </template>
            <template v-else-if="[4,5,6,9].includes(from)">
              <img class="sorry-title" src="../images/pop/congratulatoin-title.png" alt />
              </template>
            <template v-else-if="from==7"><img class="sorry-title" src="../images/pop/remind-title.png" alt /></template>
            <template v-else-if="from==8"><img class="congra-you-title" src="../images/pop/congra-you-title.png" alt /></template>
          </div>
            ...
            <div :class="getClassName('product-img')" v-if="[2,4,5,6,9].includes(from)">
              <div class="item" v-for="(item,index) in carddata">
                <div class="img_bg" :class="{double:item.doubleState}">
                  <div  :class="{level2_tag:item.awardsType=='fbk'&&level==2,level3_tag:item.awardsType==='fbk'&&level==3}">

                  </div>
                  <img
                    :src="`${require(`../images/${item.awardsType}.png`)}`"
                    alt
                    v-if="item.awardsType!='fbk'"
                  />
                ...
                <span>
                  ...
                  <i v-if="from==6" :class="{second:index==1}">
                    <br />
                    <template v-if="level==2">（限中级场使用）</template>
                    <template v-if="level==3">（限高级场使用）</template>
                  </i>
                </span>
              </div>
            </div>
          </div>
          <div class="tips" v-if="from==1">
            您暂
            <i>未上榜</i>，下次继续加油哟~
          </div>
          <div class="tips" v-if="from==2">
            恭喜您获得
            <i>{{rank}}</i>名，获得以上奖励
          </div>
          <div class="tips" v-if="from==4">继续翻牌可获得更多奖励</div>
          <div class="tips" v-if="from==5">
            翻倍卡生效，
            <i>奖励翻倍</i>
          </div>
          <div class="tips" v-if="from==6">
            继续翻出奖品
            <i>奖励x2</i>
          </div>
          <div class="tips" v-if="from==9">加赠点数将自动到账</div>
          <template>
            <div :class="getClassName('btn btn-yellow')" @click="btnyellowclick"></div>
            <div
              :class="getClassName('btn btn-pink')"
              v-if="[3,7].includes(from)"
              @click="btnpinkclick"
            ></div>
          </template>
        </div>
...
export default {
  ...
  methods: {
    ...
    btnyellowclick() {
      ...
      } else if (this.from == 7 || this.from == 11) {
        //放弃翻倍卡
...
.rule {
  ...
  .pop {
    ...
    .wrap {
      ...
      padding: 1.9rem 1rem 0.6rem 1rem;
      width: 7.1rem;
      height: 9.34rem;
      background: url("../images/pop/pop.png") no-repeat center center / 100%
        100%;
      &.flag3,
      &.flag7 {
        // padding: 1.1rem 0.75rem 0.6rem 1rem;
        width: 7.1rem;
        height: 9.34rem;
        background: url("../images/pop/pop.png") no-repeat center center / 100%
          100%;
        ...
        .item {
          ...
          .img_bg {
            ...
            // 中级场高级场右上角标签提示
              ...
              right: 0;
              top: 0;
              background: url("../images/pop/level2.png");
              background-size: 100% 100%;
            }
            .level3_tag{
               width: 0.83rem;
              ...
              right: 0;
              top: 0;
              background: url("../images/pop/level3.png");
              background-size: 100% 100%;  
            }
            &.double:before {
              content: "";
