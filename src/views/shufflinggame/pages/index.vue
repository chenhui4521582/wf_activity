<template>
  <div class="eggs-container" :class="{header1:level==1,header2:level==2,header3:level==3}">
    ...
    <!--规则-->
    <rule :ruleMain="actData.beginDate+'~'+actData.endDate+'，'+actData.rankDate" v-if="actData">
    </rule>
    <game ref="game"></game>
    <!--&lt;!&ndash;礼包&ndash;&gt;-->
    ...
    <div class="btn">
      ...
      <div class="record" @click="showrecord"
        :class="{header1:level==1,header2:level==2,header3:level==3}">已翻奖励</div>
      <!--重置-->
      <div class="reset" :class="{header1:level==1,header2:level==2,header3:level==3}"
        @click="resetClick"></div>
    </div>
    <!--翻牌区域-->
    <card :level="level" :isReset="isReset" @reset="isReset=false" :cardData="styleItemsArr"
      v-if="userData&&styleItemsArr.length" @refreshUserInfo="getUserInfo"
      @refreshCardInfo="getBetProgress" @getawards="getcardawards" :isBeginAnimate="isBeginAnimate">
    </card>
    <!--获得更多翻牌点-->
    <drop-down ref="dropDown" :level="level" :data="userData" v-if="userData"
      @getUserInfo="getUserInfo" @gotoplay="gotoplay" :end="isEnd"></drop-down>
    <!--翻牌点记录-->
    <record ref="record" @package="$refs.dropDown.handleTab(0)"></record>
    <!--通用弹窗1-10场景-->
    <com-pop v-if="flag" :from="flag" :level="level" @close="closecomPop" @sureGrade="sureGrade"
      @package="openpackage" @sureCard="sureCard" :carddata="cardawardsdata" @card="isReset=true"
      @gotoplay="gotoplay" @downpackage="$refs.dropDown.outHandleTab(0)" :rank="rank"></com-pop>
  </div>
</template>
