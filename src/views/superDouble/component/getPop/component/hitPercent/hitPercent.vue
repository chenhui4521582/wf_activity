<template>
  <div class="hit-percent">
    <div class="hb-task-box">
      <div class="percent-box" :style="{background:'#FFA200'}">
        <div class="percent-number" :style="{width:wpercent,background:'#FF4330'}">
        </div>
      </div>
      <ul>
        <li v-for="(item,index) in envelopsItem" v-if="!item.dot">
          <h2>{{type===2?'充值':'支持金叶'}}</h2>
          <h4 v-if="envelopsItem.length<6">{{item.amount | conversion(type)}}</h4>
          <template v-else>
            <h4 v-if="index<envelopsItem.length-1">{{item.amount | conversion(type)}}</h4>
            <h4 v-else>终极档位</h4>
          </template>
          <div class="hb-line">
            <img :src="require(`../../../../img/crush-icon.png`)" alt="">
          </div>
          <div class="envelopes">{{item.awards}}个</div>
          <div class="btn btn-complete" :style="{color:'#ED624F'}" v-if="item.status == 2">完成
          </div>
...
export default {
  ...
  filters: {
    conversion (value, type) {
      let string = ''
      if (value >= 10000) {
        string = `${Math.floor(value / 1000) / 10}万`
      } else {
        string = value
      }
      return type === 2 ? string + '元' : string
    }
  },
  computed: {
    envelopsItem () {
  ...
  methods: {
    ...
    gotocomplete (item) {
      GLOBALS.marchSetsPoint('A_H5PT0301003593', {
        task_id: item.sort,
        task_name: this.type === 2 ? '充值' + item.amount + '元' : '支持金叶' + item.amount
      }) // H5平台-疯狂翻倍活动-充值任务去完成点击
      if (this.type === 1) {
        this.openPop(8)
      } else {
    ...
    async gotoact (item) { // 领取
      GLOBALS.marchSetsPoint('A_H5PT0301003594', {
        task_id: item.sort,
        task_name: this.type === 2 ? '充值' + item.amount + '元' : '支持金叶' + item.amount
    ...
    openPop (type, number) {
      this.$emit('open-pop', type, number)
    }
  },
  components: {
